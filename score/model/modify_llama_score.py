...
from ..score_cache import CakeCache, CakeDecodingKVCache_LayerWise

from ..utils import get_scores_with_kv_fusion
...
def llama_attn_forward_score_modify(
    self,
    ...
    if not hasattr(self, "score_func"):
        coeffs = np.load(f"/home/sjx/ours/scorekv/experiments/LongBench/full_fit_llama3_combined15_pca4.npy")
    ...
    if self.config.decoding_evict[self.layer_idx] is None and len(past_key_value.layer_budget) == self.config.prefill_score_evict[self.layer_idx].num_layers:
    ...
    if self.config.prefill[self.layer_idx]:
        ...
        pref_score = self.score_func(self.layer_idx)
        ...
        hh_score_attn = attn_cache.mean(dim=-2)
        hh_score_dist = get_scores_with_kv_fusion(key_states, value_states, window_size=self.config.window_size[self.layer_idx])
        # Store both attention and dispersion scores for two-stage process in cache
        past_key_value.update_score(pref_score, (hh_score_attn, hh_score_dist))
        past_key_value.layer_budget.append(self.config.key_size[self.layer_idx])
        self.config.prefill[self.layer_idx] =False
        past_key_value = self.config.prefill_score_evict[self.layer_idx](past_key_value, q_len)
...
def llama_model_forward_score_modify(
    self,
