...
from score.utils import adjust_budgets, information_theoretic_token_score
...
class CakeprefillKVCache:
    ...
    def evcit_layer_kvcache(self, past_key_values, layer_idx, budget):
        ...
        evict_data = past_key_values.evict_scores[layer_idx]
        
        # Check if we have both attention and dispersion scores (two-stage process)
        if isinstance(evict_data, tuple) and len(evict_data) == 2:
            hh_score_attn, hh_score_dist = evict_data
            
            # Two-stage DATI process implementation
            scoreable_len = hh_score_attn.shape[-1]
            
            if budget >= scoreable_len:
                budget = scoreable_len
                # No eviction needed, keep all tokens
                indices = torch.arange(budget, device=hh_score_attn.device).unsqueeze(0).unsqueeze(0).expand(bsz, num_key_value_heads, -1)
            else:
                # Calculate additional tokens (n) for stage 2 refinement
                if scoreable_len > 1000:
                    extra_tokens = min(budget // 2, 16)  # Up to 50% extra, max 64
                elif scoreable_len > 500:
                    extra_tokens = min(budget // 3, 16)  # Up to 33% extra, max 32
                else:
                    extra_tokens = min(budget // 4, 16)  # Up to 25% extra, max 16
                
                candidate_budget = min(budget + extra_tokens, scoreable_len)
                
                if candidate_budget > budget and extra_tokens > 0:
                    # Stage 1: Select candidate_budget tokens with highest attention scores (most important)
                    _, attn_candidates_idx = torch.topk(hh_score_attn, candidate_budget, dim=-1, largest=True)
                    
                    # Stage 2: Among candidates, select final budget tokens with lowest dispersion (least dispersed = less important)
                    # Extract dispersion scores for candidate tokens
                    candidate_dispersion_scores = torch.gather(
                        hh_score_dist, 
                        dim=-1, 
                        index=attn_candidates_idx
                    )
             
                    # Actually, we want to keep tokens with HIGH dispersion (more important)
                    # So we select tokens with highest dispersion among candidates
                    _, final_keep_idx_in_candidates = torch.topk(
                        candidate_dispersion_scores, 
                        budget, 
                        dim=-1, 
                        largest=True  # Highest dispersion = more important = keep these
                    )
                    ...
                    indices = torch.gather(
                        attn_candidates_idx,
                        dim=-1, 
                        index=final_keep_idx_in_candidates
                    )
                else:
                    # Single-stage: use attention scores only (keep highest attention)
                    indices = torch.topk(hh_score_attn, budget, dim=-1, largest=True).indices
            
            # Update the evict scores to reflect final selection
            final_scores = torch.gather(hh_score_attn, dim=-1, index=indices)
            past_key_values.evict_scores[layer_idx] = final_scores
            
        else:
            # Original single-score logic (backward compatibility)
            hh_score = evict_data
        ...
        # Apply the selected indices to compress KV cache
...
class CakeDecodingKVCache_LayerWise:
    ...
    def __call__(self, past_key_values, attn_score_cache, layer_idx):
        ...
        # Fast scoring for decoding phase - prioritize speed over theoretical completeness
        attn_scores_for_scoring = attn_score_cache[:, :, :, :-self.window_size]
        
        # Use simple attention statistics for decoding (much faster)
        attn_cache = attn_scores_for_scoring.mean(dim=-2)  # Average over query positions
        
        # Apply light smoothing
        attn_cache = F.avg_pool1d(attn_cache, kernel_size=3, padding=1, stride=1)  # Reduced kernel size
        
        # Reshape for head grouping
        ...
        indices = attn_cache.topk(self.hh_size, dim=-1).indices  # [bsz, num_key_value_heads, hh_size]
        
        # Ensure indices has the expected shape before expansion
        if indices.dim() != 3:
            raise RuntimeError(f"Expected indices to have 3 dimensions, got {indices.dim()}, shape: {indices.shape}")
        
        # Expand indices for gathering
        indices = indices.unsqueeze(-1).expand(-1, -1, -1, head_dim)  # [bsz, num_key_value_heads, hh_size, head_dim]
